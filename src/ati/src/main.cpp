...
#include <stdlib.h>
#include "ati/Force.h"
//The factory set baud rate is 500 kbps.
...
int main(int  argc, char **argv) 
{
	...
	ros::init (argc, argv, "ati");
	//ros::Rate loop_rate(100);
	ros::NodeHandle nh;
	ros::Publisher pub_force=nh.advertise<ati::Force>("/force_msg",1000);
        ati::Force force_data;
	...
	float k=1000000.0;              /*Parameter for converting from counts to unit*/
	...
	while(ros::ok())
	{
	   ...
	   for (i =0;i < 6;i++) {
		   printf("%s: %f\n", AXES[i], resp.FTData[i]/k);
	   }
	   ati::Force force_data;
	   force_data.force_x.push_back(resp.FTData[0]/k);
	   force_data.force_y.push_back(resp.FTData[1]/k);
	   force_data.force_z.push_back(resp.FTData[2]/k);
	   force_data.torque_x.push_back(resp.FTData[3]/k);
	   force_data.torque_y.push_back(resp.FTData[4]/k);
	   force_data.torque_z.push_back(resp.FTData[5]/k);
	   pub_force.publish(force_data);
